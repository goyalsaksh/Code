...
void CitizenQueueList::enqueue(Citizen *new_citizen)
{
     ...
     */
    if (front == NULL) // First registration
    {
        front = new_citizen;
        rear = new_citizen;
    }
    else
    {
        ...
        while (cur != NULL && cur->get_priority() >= new_citizen->get_priority()) // Finding the location for insertion
        {
            prev = cur;
            cur = cur->next;
        }
        if (prev == NULL) // Appending to the head
        {
            new_citizen->next = front;
            front = new_citizen;
        }
        else if (cur == NULL) // Appending to the rear
        {
...
void CitizenQueueList::remove(Citizen *user, string pswd)
{
    ...
    Citizen *cur = front;
    Citizen *prev = NULL;
    while (cur->citizen_id != user->citizen_id && cur != NULL)
    { // Finding the person in the queue-list
        prev = cur;
        cur = cur->next;
    }
    if (prev == NULL) // Removing from the front
        front = cur->next;
...
void CitizenQueueList::display_top_n(unsigned n)
{
    ...
    Citizen *user = front;
    unsigned count;
    for (count = 1; count <= n && user != NULL; count++)
    {
